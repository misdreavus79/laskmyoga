    ...
    <?php
        if( has_post_thumbnail() ) {
          $image = '';              
            ...
            $image .= get_the_post_thumbnail( $post->ID, 'featured-full', array( 'title' => esc_attr( $title_attribute ), 'alt' => esc_attr( $title_attribute ) ) ).'</a>';
...
add_action('interface_benefits_of_yoga_template_content', 'interface_display_benefits_of_yoga_template_content');
/**
 * Fuction to show the benefits of yoga section.
 */
function interface_display_benefits_of_yoga_template_content() {
  global $post; 
  global $options, $array_of_default_settings;
 $options = wp_parse_args( get_option( 'interface_theme_options', array() ), interface_get_option_defaults());
  ...
  <article>
    ...
    <div class="entry-content clearfix">
      ...
      <?php 
            ...
            the_content(); 
            $poses = get_field('pose');
            $count = 0;
            $tileWidth = [60, 40, 50, 50, 55, 45];
    ?>
            <div class="poses">
              <?php foreach($poses as $p) : ?>
                <figure class="pose pose-<?=$tileWidth[$count];?>">
                    <div class="pose-meta">
                      <img src="<?= $p['image']; ?>" alt="Benefits of Yoga Poses - <? $m['name']; ?>">
                      <figcaption class="pose-name"><?= $p['name']; ?></figcaption>
                    </div>
                    <p class="pose-description"><?= $p['description']; ?></p>
                </figure>
                <?php 
                  $count++;
                  if($count == 6){
